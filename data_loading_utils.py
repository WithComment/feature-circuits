...
def load_rct(
  ...
  for line in dataset_items:
    ...
    clean_prompt = model.tokenizer(data["clean_prompt"]).input_ids
    ...
    if max_length is not None and len(clean_prompt) > max_length:
      continue

    if use_min_length_only:
      # restart collection if we've found a new shortest example
      if (length := len(clean_prompt)) < min_length:
