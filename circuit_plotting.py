...
def plot_circuit(
  ...
  # Create graph
  G = Digraph(name="Feature circuit")
  # newrank needed for proper alignment
  G.graph_attr.update(rankdir="BT", newrank="true")
  G.node_attr.update(shape="box", style="rounded")
  ...
  for submod, values in nodes.items():
    ...
    for feat_idx in range(values.shape[0]):
      if abs(values[feat_idx]) > node_threshold:
        ...
        width = max(CHAR_WIDTH * len(label_line1),
                    CHAR_WIDTH * len(label_line2)) + NODE_BUFFER
        node_widths[name] = width
  ...
  for i, submod in enumerate(components):
    ...
    if nodes_in_layer:
      total_width = sum(node_widths[n] for n in nodes_in_layer) + \
          INTERNODE_BUFFER * (len(nodes_in_layer) - 1)
  ...
  # Add edges
  for upstream_mod, downstream_dict in tqdm(edges.items()):
