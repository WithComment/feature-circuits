...
def _load_pythia_saes_and_submodules(
  ...
  assert len(
      model.gpt_neox.layers) == 6, "Not the expected number of layers for pythia-70m-deduped"
...
def _load_gemma_saes_and_submodules(
  ...
  assert len(
      model.model.layers) == 26, "Not the expected number of layers for Gemma-2-2B"
  if thru_layer is None:
...
def test_submodule_shapes(model, submodule):
  ...
  assert t.allclose(
      out1.value, out2.value), f"Submodule {submodule.name}: Output should be the same before and after grabbing acts/grads"
  assert len(
      acts.shape) == 3, f"Submodule {submodule.name}: Expected 3D tensor [batch, seq_len, dim], got shape {acts.shape}"
...
def test_activation_intervention(model, submodule):
  ...
  assert not t.allclose(
      out1.value, out2.value), f"Submodule {submodule.name}: Output should change after modifying activation"
...
def test_gradient_intervention(model, upstream_submodule, downstream_submodule):
  ...
  assert t.allclose(
      out1.value, out2.value), f"Submodule {upstream_submodule.name}: Output should be same before and after stopping gradients"
  assert not t.allclose(
      grads1.value, grads2.value), f"Submodule {upstream_submodule.name}: Gradients should change after stopping grad flow in {downstream_submodule.name}"


def run_tests(model):
  """Run all tests for the Submodule class"""
  submodules, _ = load_saes_and_submodules(
      model, neurons=True, include_embed=False)  # Use neurons=True to avoid loading dictionaries
  ...
  for upstream, downstream in gradient_test_pairs:
    print(
        f"Testing gradient intervention from {upstream.name} to {downstream.name}...")
