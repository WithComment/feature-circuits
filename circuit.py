...
def get_circuit(
  ...
  effects, deltas, grads, total_effect = patching_effect(
      ...
      method=args.method,  # get better approximations for early layers by using ig
  )
...
def get_circuit_cluster(
  ...
  if model_name == "EleutherAI/pythia-70m-deduped":
    model = LanguageModel(model_name, device_map=device,
                          dispatch=True, torch_dtype=dtype)
  ...
  for batch in tqdm(batches, desc="Batches"):
    ...
    if running_nodes is None:
      ...
      running_edges = {
          k: {kk: len(batch) * edges[k][kk].to("cpu")
  ...
  edges = {
      k: {kk: 1 / num_examples * v.to(device)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--method",
                      type=str,
                      default='ig',
                      help="Method for calculating IE. One of 'atp', 'ig', 'exact'.")
      ...
      match args.mode:
        ...
        case 'pair':
          ...
          patch_answer_idxs = t.tensor(
              [model.tokenizer(e["patch_answer"]).input_ids[-1]
               for e in batch],
              dtype=t.long,
              device=device,
          )
